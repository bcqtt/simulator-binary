...
public class ByteArrayConveter {
	...
	// 从byte数组的index处的连续4个字节获得一个int，小端序
	public static int getIntSmallEnd(byte[] arr, int index) {
		return (0xff & (arr[index + 0] )) 
				| (0xffff & (arr[index + 1] << 8))
				| (0xffffff & (arr[index + 2] << 16)) 
				| (arr[index + 3]<< 24);
	}
	...
	// 从byte数组的index处的连续4个字节获得一个float
	public static float getFloatSmallEnd(byte[] arr, int index) {
		return Float.intBitsToFloat(getIntSmallEnd(arr, index));
	}
	...
	public static void main(String[] args) {
		byte[] arr = {(byte)0xc2,(byte)0xa5,(byte)0xb2,(byte)0x46};
		System.out.println(getFloatSmallEnd(arr, 0));
	}
}
